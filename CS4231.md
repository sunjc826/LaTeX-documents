...
Given history $H$, suppose that it is locally linearizable. Let $x_1,x_2,\dots,x_m$ be the objects that appear in $H$. Restating the assumption, $\forall i\in \{1,\dots,m\}, H|x_i$ is linearizable, such that there is a choice function $c_i = o \mapsto c_i(o)\in [inv(o, x_i), resp(o, x_i)]$.

Let $c$ be the "union" of all $c_i$. Immediately, we see that $c$ is a well-defined choice function for all operations $o$ in $H$. We claim that $c$ is a legal linearization of $H$.
...
We discuss some aspects of the proof in lecture (using definition 2).
