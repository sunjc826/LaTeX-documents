...
**Proof idea** Make use of happened before order of receive events.
...
Definition: A "last" message $m$ is one for which $recv(m)$ does not happen before any send-receive event on $node(recv(m))$.
...
The proof of validity is given in lecture and as it is simple, it does not necessitate transcription.
