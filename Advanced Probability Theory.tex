...
\paragraph{Ex 2.5.6} Suppose $P$ satisfies finite additivity, and that given a monontonically decreasing set sequence $(A_i)$, with each $A_i$ being a finite disjoint union of elements of $\semialg$ and $\cap_n A_n = \emptyset$, that $\lim_{n\rightarrow \infty}P(A_n) = 0$.
...
\subparagraph{b} (TODO) Suppose we have $N$ disjoint sets $\{D_1,\dots,D_n\}$.

Following the hint, letting $k\in \mathbb{N}$ be the number that the results of only coins $1$ to $k$ are specified by any $D_n, n \leq N$, we partition $\Omega$ into $2^k$ subsets $A_{a_1,\dots,a_k}$, $a_1,\dots,a_k\in\{0,1\}$.
